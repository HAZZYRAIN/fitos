...
// Safe number helper — prevents toLocaleString / toFixed crashes on undefined
...
export default function DietHabits() {
  ...
  if (dh.length === 0) return (
    <>
      ...
      <div className="alert al-b">No habit logs for {dietClient || "this client"} yet. Log the first entry below.</div>
      <div className="card">
        ...
        <div className="field"><label>Sleep Quality</label>
          <select className="fi" value={newDiet.sleepQuality} onChange={(e) => setNewDiet((p: any) => ({ ...p, sleepQuality: e.target.value }))}>
        ...
        <div className="field"><label>Notes (optional)</label>
          <textarea className="fi" rows={2} placeholder="Ate out, stress, travel..." value={newDiet.notes} onChange={(e) => setNewDiet((p: any) => ({ ...p, notes: e.target.value }))} style={{ resize: "none" }} />
        </div>
  ...
  // Safe averages — n() guards against undefined values in old data
  ...
  return (
    <>
      ...
      <div className="g4">
        {[
          { l: "Avg Protein", v: `${avgProtein}g`,             target: `Target: ${PROTEIN_TARGET}g`, pct: Math.min(100, Math.round(avgProtein / PROTEIN_TARGET * 100)),      c: "var(--brand)" },
          { l: "Avg Water",   v: `${avgWater}L`,               target: `Target: ${WATER_TARGET}L`,   pct: Math.min(100, Math.round(Number(avgWater) / WATER_TARGET * 100)),  c: "var(--blue)" },
          { l: "Avg Steps",   v: n(avgSteps).toLocaleString(), target: `Target: ${STEPS_TARGET.toLocaleString()}`, pct: Math.min(100, Math.round(avgSteps / STEPS_TARGET * 100)), c: "var(--green)" },
          { l: "Avg Sleep",   v: `${avgSleep}h`,               target: `Target: ${SLEEP_TARGET}h`,   pct: Math.min(100, Math.round(Number(avgSleep) / SLEEP_TARGET * 100)),  c: "var(--purple)" },
        ].map((s, i) => (
          <div key={i} className="sc">
            ...
            <div className="pw mt8"><div className={`pb ${s.pct >= 90 ? "pb-g" : s.pct >= 70 ? "pb-y" : "pb-r"}`} style={{ width: `${s.pct}%` }} /></div>
      ...
      <div className="g2">
        ...
        <div className="card">
          ...
          <div className="field"><label>Notes (optional)</label>
            <textarea className="fi" rows={2} placeholder="Ate out, stress, travel, period, illness..." value={newDiet.notes} onChange={(e) => setNewDiet((p: any) => ({ ...p, notes: e.target.value }))} style={{ resize: "none" }} />
          </div>
          <button className="btn btn-p btn-s mt8" style={{ width: "100%" }} onClick={saveDiet}>Save Habit Log</button>
        </div>
        ...
        <div className="col gap14">
          <div className="card">
            ...
            <LineChart data={dh.map((d: any) => n(d.protein))} color="var(--brand)" />
          </div>
          <div className="card">
            ...
            <LineChart data={dh.map((d: any) => n(d.sleep))} color="var(--purple)" />
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0 }}>
        <div style={{ padding: "16px 20px 0" }}><span className="ct">Habit Log History</span></div>
        <div className="tw">
          <table>
            <thead>
              <tr><th>Date</th><th>Protein</th><th>Water</th><th>Steps</th><th>Sleep</th><th>Quality</th><th>Notes</th></tr>
            </thead>
            <tbody>
              {dh.slice().reverse().map((d: any, i: number) => (
                <tr key={i}>
                  <td className="fw6">{d.date}</td>
                  <td><span className={n(d.protein) >= PROTEIN_TARGET * 0.9 ? "tg fw7" : "tr fw7"}>{n(d.protein)}g</span></td>
                  <td><span className={n(d.water) >= WATER_TARGET * 0.9 ? "tg fw7" : "ty fw7"}>{n(d.water)}L</span></td>
                  <td><span className={n(d.steps) >= STEPS_TARGET * 0.8 ? "tg" : "ty"}>{n(d.steps).toLocaleString()}</span></td>
                  <td><span className={n(d.sleep) >= 7 ? "tg" : "ty"}>{n(d.sleep)}h</span></td>
      ...
      <div className="card">
        ...
        <div className="col gap8">
          ...
          {avgSteps < STEPS_TARGET * 0.7 && <div className="alert al-y">🚶 Steps averaging {n(avgSteps).toLocaleString()} vs 10,000 target.</div>}
          {avgProtein >= PROTEIN_TARGET * 0.9 && Number(avgSleep) >= 7 && avgSteps >= STEPS_TARGET * 0.8 && (
            <div className="alert al-g">✓ All habits on track. Acknowledge this in next session.</div>
          )}
        </div>
      </div>
    </>
