...
export function AdminProvider({
  ...
  const [newInstruction, setNewInstruction] = useState<Partial<Instruction>>({ title: "", body: "", priority: "medium" });
  ...
  const filteredClients = useMemo(() => clients.filter((c) => {
    const matchesSearch  = c.name?.toLowerCase().includes(clientSearch.toLowerCase()) || c.trainerName?.toLowerCase().includes(clientSearch.toLowerCase());
  ...
  const addTrainer = useCallback(async () => {
    if (!newTrainer.name?.trim() || !newTrainer.email?.trim()) { showToast("Name and email required", "error"); return; }
    try {
      await addDoc(collection(db, "trainers"), {
        name: newTrainer.name.trim(), email: newTrainer.email.trim(),
        speciality: newTrainer.speciality || "", plan: newTrainer.plan || "Starter",
        status: "active", rating: 0, sessions: 0, clientCount: 0,
        lateSubmissions: 0, accountabilityScore: 100, revenue: 0, retention: 0,
        warnings: 0, missedSessions: 0, pendingLogs: 0, progressUpdatesThisMonth: 0,
        sessionsAssigned: 0,
    ...
    } catch { showToast("Failed to add trainer", "error"); }
  }, [newTrainer, showToast]);

  const addClient = useCallback(async () => {
    if (!newClient.name?.trim() || !newClient.trainerId) { showToast("Name and trainer required", "error"); return; }
    ...
    try {
      const docRef = await addDoc(collection(db, "trainers", trainerId, "clients"), {
        name: clientName, email: newClient.email?.trim() || "",
        gender: newClient.gender || "", age: newClient.age ? Number(newClient.age) : null,
        trainerId, trainerName: newClient.trainerName || "",
        programType: newClient.programType || "1-on-1", status: newClient.status || "Active",
        medicalNotes: newClient.medicalNotes || "", startDate: newClient.startDate || "",
        endDate: newClient.endDate || "", plan: newClient.plan || "",
        location: newClient.location || "", sessionsIncluded,
        sessionsLogged: 0, classesLeft: sessionsIncluded,
        compliance: 0, missedSessions: 0, lastSession: null, lateLog: false,
        createdAt: serverTimestamp(),
      });
  ...
  const openEditClient = useCallback((client: Client) => {
    setEditForm(client); setShowEditClient(true);
  }, []);

  const saveEditClient = useCallback(async () => {
    if (!editForm?.id || !editForm?.trainerId) return;
    try {
      await updateDoc(doc(db, "trainers", editForm.trainerId, "clients", editForm.id), { ...editForm, updatedAt: serverTimestamp() });
      setShowEditClient(false); setEditForm(null);
      showToast("✓ Client updated", "success");
    } catch { showToast("Failed to update client", "error"); }
  }, [editForm, showToast]);

  const openRenewClient = useCallback((client: Client) => {
    setRenewTarget(client); setShowRenewClient(true);
    setRenewForm({ mode: "add", sessions: "", endDate: "" }); setRenewMsg("");
  }, []);

  const renewClient = useCallback(async () => {
    if (!renewTarget?.id || !renewTarget?.trainerId || !renewForm.sessions || !renewForm.endDate) { setRenewMsg("Please fill all fields"); return; }
    ...
    try {
      ...
      if (renewForm.mode === "add") {
        await updateDoc(clientRef, { sessionsIncluded: (renewTarget.sessionsIncluded || 0) + newSessions, classesLeft: (renewTarget.classesLeft || 0) + newSessions, endDate: renewForm.endDate, updatedAt: serverTimestamp() });
      } else {
        await updateDoc(clientRef, { sessionsIncluded: newSessions, sessionsLogged: 0, classesLeft: newSessions, compliance: 0, missedSessions: 0, endDate: renewForm.endDate, updatedAt: serverTimestamp() });
      }
      setRenewMsg("✓ Renewal complete!");
      setTimeout(() => { setShowRenewClient(false); setRenewTarget(null); }, 1500);
    } catch { setRenewMsg("✕ Failed to renew"); }
  ...
  const postInstruction = useCallback(async () => {
    ...
    try {
      await addDoc(collection(db, "instructions"), { ...newInstruction, createdAt: serverTimestamp() });
      setNewInstruction({ title: "", body: "", priority: "medium" }); setShowInstruction(false);
      showToast("✓ Instruction posted", "success");
    } catch { showToast("Failed to post instruction", "error"); }
  }, [newInstruction, showToast]);

  const addWarning = useCallback(async () => {
    if (!newWarning.trainer?.trim() || !newWarning.note?.trim()) { showToast("Trainer and note required", "error"); return; }
    try {
      await addDoc(collection(db, "warnings"), { ...newWarning, createdAt: serverTimestamp() });
      setNewWarning({ trainer: "", type: "Verbal Warning", note: "", followUp: "" }); setShowWarning(false);
      showToast("✓ Warning logged", "success");
    } catch { showToast("Failed to log warning", "error"); }
  }, [newWarning, showToast]);

  const changePassword = useCallback(async () => {
    if (!pwForm.newPw || !pwForm.confirmPw || pwForm.newPw !== pwForm.confirmPw) { setPwMsg("✕ Passwords don't match"); return; }
    if (pwForm.newPw.length < 6) { setPwMsg("✕ Minimum 6 characters"); return; }
    setPwMsg("✓ Use Firebase Console → Authentication to change passwords in production");
  }, [pwForm]);

  const toggleTrainerStatus = useCallback(async (trainerId: string, trainerName: string, newStatus: string) => {
    try {
      await updateDoc(doc(db, "trainers", trainerId), { status: newStatus, updatedAt: serverTimestamp() });
      showToast(`✓ ${trainerName} marked ${newStatus}`, "success");
    } catch { showToast("Failed to update trainer status", "error"); }
  }, [showToast]);
  ...
  const value: AdminContextType = {
    ...
    filteredClients, atRiskClients, flaggedClients, lowAttendance,
    expiredClients, lowClassClients, pendingLogs,
    ...
    renewClient, openRenewClient, openEditClient,
    postInstruction, addWarning, changePassword,
    toggleTrainerStatus, deleteInstruction,
    tab, setTab,
  };
