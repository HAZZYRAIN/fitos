...
export const S = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sh-l h2 { font-family:var(--fd); font-size:20px; font-weight:800; letter-spacing:-0.3px; }
  ...
  /* ── MODALS ── */
  .overlay { position:fixed; inset:0; background:rgba(0,0,0,0.75); display:flex; align-items:center; justify-content:center; z-index:999; backdrop-filter:blur(4px); }
  .modal { background:var(--s1); border:1px solid var(--b2); border-radius:18px; padding:28px; width:460px; max-height:90vh; overflow-y:auto; box-shadow:0 32px 80px rgba(0,0,0,0.8); }
  .modal-lg { width:640px; }
  .modal-t { font-family:var(--fd); font-size:16px; font-weight:800; margin-bottom:18px; }

  /* ── FORMS ── */
  .field { display:flex; flex-direction:column; gap:5px; margin-bottom:12px; }
  .field label { font-size:10px; color:var(--t3); font-weight:700; text-transform:uppercase; letter-spacing:1px; }
  .fi { background:var(--s2); border:1px solid var(--b2); border-radius:var(--rs); padding:9px 12px; color:var(--t1); font-size:13px; font-family:var(--fb); outline:none; transition:border-color 0.2s; width:100%; }
  .fi:focus { border-color:rgba(255,77,0,0.4); }
  .fi::placeholder { color:var(--t4); }
  select.fi { cursor:pointer; }

  /* ── ALERTS ── */
  .alert { padding:10px 14px; border-radius:var(--rs); font-size:12px; font-weight:500; }
